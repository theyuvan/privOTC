...
// node_modules/@chainlink/cre-sdk/dist/generated/capabilities/networking/confidentialhttp/v1alpha/client_pb.js
...
// node_modules/@chainlink/cre-sdk/dist/generated-sdk/capabilities/networking/confidentialhttp/v1alpha/client_sdk_gen.js
...
// ../contracts/abi/OTCSettlement.ts
var OTCSettlement = [
  {
    inputs: [
      {
...
var configSchema = exports_external.object({
  ...
  otcSettlementAddress: exports_external.string(),
  ...
  chainId: exports_external.string().optional(),
  tenderlyRpcUrl: exports_external.string().optional(),
...
class ConfidentialOrderbook {
  ...
  addIntent(intent) {
    ...
    orders.sort((a, b) => {
      ...
      return intent.side === "buy" ? priceB - priceA || a.timestamp - b.timestamp : priceA - priceB || a.timestamp - b.timestamp;
    });
    this.usedNullifiers.add(intent.worldIdNullifier);
    return { success: true };
  }
  findMatches(tokenPair) {
...
async function validateZKProof(runtime2, zkProof) {
  try {
    ...
    if (config.simulationMode) {
      ...
      const proofStr = JSON.stringify(proofObj2);
      let mockCount = 0;
      for (const pattern of mockPatterns) {
        if (proofStr.includes(`"${pattern}"`))
          mockCount++;
      }
      if (mockCount >= 4) {
        runtime2.log("❌ Rejected mock ZK proof (detected test values)");
      ...
      runtime2.log("✅ ZK proof structure validated (REAL Groth16 proof)");
      return {
        success: true,
...
var handleTradeIntake = async (runtime2, payload) => {
  ...
  runtime2.log("2️⃣  Validating ZK balance proof...");
  const zkResult = await validateZKProof(runtime2, zkProof);
  if (!zkResult.success) {
    runtime2.log(`❌ ZK proof validation failed: ${zkResult.reason}`);
    return { statusCode: 400, body: { success: false, reason: zkResult.reason } };
  }
  runtime2.log("✅ World ID and ZK proof verified");
  runtime2.log("3️⃣  Adding to confidential orderbook...");
  const intent = {
  ...
  if (!addResult.success) {
    runtime2.log(`❌ Failed to add to orderbook: ${addResult.reason}`);
    return { statusCode: 400, body: { success: false, reason: addResult.reason } };
  }
  const depth = orderbook.getDepth(trade.tokenPair);
  runtime2.log(`✅ Trade intent added | Orderbook depth: ${depth.buys} buys, ${depth.sells} sells`);
  return {
    statusCode: 200,
    body: {
      success: true,
...
var executeSettlement = (runtime2, match) => {
  runtime2.log(`\uD83D\uDCB1 Executing on-chain settlement for match ${match.buyOrder.id.substring(0, 8)}...`);
  runtime2.log(`   Pair: ${match.buyOrder.tokenPair}`);
  runtime2.log(`   Amount: ${match.matchAmount} @ ${match.matchPrice}`);
  const config = runtime2.config;
  try {
    ...
    const matchIdHash = `0x${Array.from(matchIdBytes.slice(0, 32)).map((b) => b.toString(16).padStart(2, "0")).join("").padEnd(64, "0")}`;
    const toBytes32 = (value2) => {
      try {
        return `0x${BigInt(value2).toString(16).padStart(64, "0")}`;
      } catch {
        const hex = value2.startsWith("0x") ? value2.slice(2) : Array.from(new TextEncoder().encode(value2)).map((b) => b.toString(16).padStart(2, "0")).join("");
    ...
    const settlementData = encodeFunctionData({
      ...
      args: [
        ...
        BigInt(Math.floor(parseFloat(match.matchAmount) * 1000000000000000000)),
        BigInt(Math.floor(parseFloat(match.matchPrice) * 1e6)),
        config.otcSettlementAddress
      ]
    });
    runtime2.log(`   \uD83D\uDCDD Settlement transaction data prepared`);
    runtime2.log(`      Match ID: ${matchIdHash.slice(0, 10)}...`);
    runtime2.log(`      Amount: ${match.matchAmount} tokens`);
    runtime2.log(`      Price: ${match.matchPrice} USDC`);
    runtime2.log(`      Settlement contract: ${config.otcSettlementAddress}`);
    if (config.simulationMode) {
      runtime2.log(`   Buyer commitment: ${match.buyOrder.walletCommitment.substring(0, 16)}...`);
      runtime2.log(`   Seller commitment: ${match.sellOrder.walletCommitment.substring(0, 16)}...`);
      runtime2.log(`   Gas limit: ${config.gasLimit || "500000"}`);
      runtime2.log(`   ✅ Settlement prepared (SIMULATION MODE - no tx sent)`);
      runtime2.log(`      To execute in production: set simulationMode=false and deploy contract`);
      return { txHash: matchIdHash };
    }
    runtime2.log(`   \uD83D\uDE80 Sending settlement transaction to ${config.chainName}...`);
    ...
    const resp = evmClient.writeReport(runtime2, {
      receiver: config.otcSettlementAddress,
    ...
    runtime2.log(`   ✅ Settlement executed on-chain!`);
    runtime2.log(`      Transaction hash: ${txHashHex}`);
